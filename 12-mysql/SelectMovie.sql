...
SELECT * FROM movie ORDER BY date DESC LIMIT 1;

-- Récupère le film le plus ancien
SELECT * FROM movie ORDER BY date ASC LIMIT 1;

-- Récupère le film le plus récent et le plus ancien
SELECT * FROM movie
WHERE date = (SELECT MAX(date) FROM movie)
OR    date = (SELECT MIN(date) FROM movie);
